    ...
    try:
        ...
        for asset in assets:
            ...
            try:
                ...
                install_deps(bot_dir, background=True)
...
def install_deps(bot_dir, background=False):
    def do_install():
        bot_type = detect_bot_type(bot_dir)
        bot_name = bot_dir.name
        try:
            # Skip install if node_modules already exists OR if preinstalled packages available
            node_path = os.environ.get('NODE_PATH', '')
            if bot_type == 'node' and ((bot_dir / 'node_modules').exists() or node_path):
                print(f'Skipping npm install for {bot_name} - using existing packages')
                bot_status[bot_name] = 'stopped'
                notify_ws(bot_name, {'type': 'install_done', 'status': 'stopped'})
                return
            (bot_dir / '.installing').write_text('1')
            bot_status[bot_name] = 'installing'
            notify_ws(bot_name, {'type': 'status', 'status': 'installing'})
            if bot_type == 'python' and (bot_dir / 'requirements.txt').exists():
                ...
                subprocess.run(['npm', 'install', '--legacy-peer-deps'],
                              cwd=bot_dir, capture_output=True, timeout=600)
            f = bot_dir / '.installing'
            if f.exists(): f.unlink()
            bot_status[bot_name] = 'stopped'
            notify_ws(bot_name, {'type': 'install_done', 'status': 'stopped'})
            print(f'Done installing {bot_name}')
        except Exception as e:
            print(f'Install failed {bot_name}: {e}')
            f = bot_dir / '.installing'
            if f.exists(): f.unlink()
            bot_status[bot_name] = 'stopped'
            notify_ws(bot_name, {'type': 'install_done', 'status': 'stopped'})
...
def upload():
    ...
    return jsonify({
        ...
        'install_log': ['⏳ Installing deps in background — wait 2 mins before clicking Start', '✅ Saved to GitHub' if gh_saved else '⚠ GitHub save failed']
    })
...
@app.route('/bot/phone/<bot_name>', methods=['POST'])
@login_required
def set_phone(bot_name):
    phone = request.json.get('phone', '').replace(' ', '').replace('+', '')
    if not phone:
        return jsonify({'error': 'No phone number'}), 400
    bot_dir = Path(BOTS_FOLDER) / bot_name
    (bot_dir / '.phone').write_text(phone)
    return jsonify({'success': True})

@app.route('/bot/input/<bot_name>', methods=['POST'])
@login_required
def bot_input(bot_name):
    if bot_name not in running_bots:
